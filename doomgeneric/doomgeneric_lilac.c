...
void disableRawMode()
{
  ...
  ioctl(KeyboardFd, KBDSMODE, KB_STD);
}
...
void DG_Init()
{
    ...
    if (FrameBufferFd >= 0)
    {
        ...
        FrameBuffer = mmap(NULL, s_ScreenWidth * s_ScreenHeight * 4, PROT_READ | PROT_WRITE, MAP_PRIVATE, FrameBufferFd, 0);
    ...
    KeyboardFd = open("/dev/kbd0", O_RDONLY | O_NONBLOCK);
    if (KeyboardFd >= 0) { // set keyboard to scancode mode
        if (ioctl(KeyboardFd, KBDSMODE, KB_RAW) < 0) {
            perror("ioctl");
            exit(1);
        }
    } else {
